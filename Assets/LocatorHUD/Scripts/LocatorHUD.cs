...
{
	public Camera newCurrentCamera;
    ...
    public void Reset()
    {
        ...
        directionTexture[(int)Direction.Left] = (Texture2D)Resources.Load("ArrowLeft");
        directionTexture[(int)Direction.Right] = (Texture2D)Resources.Load("ArrowRight");
        directionTexture[(int)Direction.Up] = (Texture2D)Resources.Load("ArrowUp");
        directionTexture[(int)Direction.Down] = (Texture2D)Resources.Load("ArrowDown");
		directionTexture[(int)Direction.Left_Up] = (Texture2D)Resources.Load("ArrowLeft");
		directionTexture[(int)Direction.Right_Up] = (Texture2D)Resources.Load("ArrowUp");
		directionTexture[(int)Direction.Left_Down] = (Texture2D)Resources.Load("ArrowDown");
		directionTexture[(int)Direction.Right_Down] = (Texture2D)Resources.Load("ArrowRight");
    ...
    {
        return newCurrentCamera.WorldToViewportPoint(transform.position);
    }
    ...
    protected Vector3 GetViewportPosAdvanced(float lerpAmount, Vector3 currentViewportPos)
    {
        ...
        Vector3 dirFromCamera = (transform.position - newCurrentCamera.transform.position).normalized;
        Quaternion dirQuat = Quaternion.LookRotation(dirFromCamera);
        float horizAngle = Mathf.DeltaAngle(newCurrentCamera.transform.eulerAngles.y, dirQuat.eulerAngles.y);
        float vertAngle = -Mathf.DeltaAngle(newCurrentCamera.transform.eulerAngles.x, dirQuat.eulerAngles.x);

        //calculate horizontal perc
        float horizFov = newCurrentCamera.fov * newCurrentCamera.aspect;
        ...
        float vertFov = newCurrentCamera.fov;
        float vertPerc = 0.0f;
	...
	void Start()
	{
		newCurrentCamera = GameObject.FindGameObjectWithTag("PlanetCamera").GetComponent<Camera>();
	}
    ...
    protected void OnGUI()
    {
        ...
        if (newCurrentCamera != null)
        {
            ...
            Vector3 screenPos = new Vector3(Mathf.Lerp(0.0f, newCurrentCamera.pixelWidth, Mathf.Clamp(viewportPos.x, 0.0f, 1.0f)),
                                            Mathf.Lerp(0.0f, newCurrentCamera.pixelHeight, Mathf.Clamp(viewportPos.y, 0.0f, 1.0f)),
                                            viewportPos.z);
            ...
            else if (screenPos.x >= newCurrentCamera.pixelWidth - horizontalSpacing)
            {
                if (direction == Direction.On_Screen)
                    direction = Direction.Right;
                horizontalOffset = Direction.Right;
                screenPos.x = newCurrentCamera.pixelWidth;
            }
            ...
            if (screenPos.y >= newCurrentCamera.pixelHeight - verticalSpacing)
            {
                if (direction == Direction.On_Screen)
                    direction = Direction.Up;
                verticalOffset = Direction.Up;
                screenPos.y = newCurrentCamera.pixelHeight;
            }
    ...
    protected void DisplayDirection(Vector3 screenPos, Direction direction, Direction horizontalOffset, Direction verticalOffset)
    {
        ...
        if (directionDistScalingEnabled[index])
        {
            ...
            screenPos.z = Vector3.Distance(newCurrentCamera.transform.position, transform.position);
        ...
        Rect rect = new Rect(screenPos.x - halfSize.x + offset.x,
                             (newCurrentCamera.pixelHeight - screenPos.y) - halfSize.y + offset.y,
                             size.x,
                             size.y);
        ...
        if (direction != Direction.On_Screen)
        {
            ...
            else if (rect.xMax > newCurrentCamera.pixelWidth - horizontalSpacing)
                rect.x -= rect.xMax - (newCurrentCamera.pixelWidth - horizontalSpacing);
            if (rect.yMin < verticalSpacing)
                rect.y += verticalSpacing - rect.yMin;
            else if (rect.yMax > newCurrentCamera.pixelHeight - verticalSpacing)
                rect.y -= rect.yMax - (newCurrentCamera.pixelHeight - verticalSpacing);
        }
